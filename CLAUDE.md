...
1. **`components/sections/apply.tsx`** — RHF + Zod form. Validates against `applyFormSchema` (form-side: `countryCode` + digits-only `whatsapp`). Fields: `assetType`, `stage`, `timeline`, `budget`, plus name/email/whatsapp/company/description. The WhatsApp control is a composite — a country-code Select (default `+971`, list in `lib/country-codes.ts`) glued to a digits/spaces input; the two values are combined into `+971 50 123 4567` format **on submit** before POSTing. Honeypot field `website` must stay empty (bots fill it; backend silently 200s).
2. On submit, the form generates a single **`eventID`** (`lead_<ts>_<rand>`) and computes a numeric **`leadValue`** from the `BUDGET_TO_VALUE` map (`$60K-$95K`→60000, `$95K-$150K`→95000, `$150K+`→150000, `Not RWA — different budget`→30000, default 60000). Both travel in the request body to `/api/apply` **and** are passed to `trackLead` for the browser Pixel.
3. **`app/api/apply/route.ts`** (Node runtime) — validates the combined whatsapp string against `applySchema` (server-side: requires leading `+` country code). Zod strips unknown keys, so `eventID`, `value`, and `countryCode` are read off the raw JSON when needed. **Notion is the only persistence layer** — `appendLead()` runs first; if it fails, the route returns 202 with a "WhatsApp us directly" message. Honeypot trip → 200 silently. Resend has been removed; do not re-add it without explicit instruction.
4. **`lib/notion.ts`** — durable lead store via Notion's API. `appendLead()` writes to a database whose schema is documented in the file's header (case-sensitive property names, must match exactly). Stage column is hardcoded to `New`; the user advances leads manually through `In review` / `Closed`. Notion notifications on the database replace the email channel that Resend used to provide.
5. **`lib/meta-capi.ts`** — server-side Meta CAPI events. Hashes email and phone (lowercase, digits-only for phone) with SHA-256. `sendMetaEvent({ eventName, ... })` is generic; the route calls it with `eventName: "Lead"`. Forwards `_fbp` / `_fbc` cookies unhashed (Meta wants raw values). No-op if `NEXT_PUBLIC_META_PIXEL_ID` or `META_CAPI_ACCESS_TOKEN` is missing. When `META_CAPI_TEST_EVENT_CODE` is set, events route to Events Manager → Test Events tab instead of counting toward production — **remove this env var when done testing**.
...
- `NOTION_API_KEY` + `NOTION_LEADS_DATABASE_ID` — durable lead store. Without both, the form returns 202 and the lead is **not persisted anywhere** (Resend was removed). Set Notion's row notifications on the database to get pinged.
